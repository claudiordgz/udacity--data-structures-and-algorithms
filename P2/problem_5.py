...
def test_case(trie):
    ...
    print('is correct for a? ', suffixes == ['nthology', 'ntagonist', 'ntonym', 'nt'], suffixes)
    node = trie.find('f')
    suffixes = node.suffixes()
    print('is correct for f? ', suffixes == ['unction','un','actory'], suffixes)


def test_case_longer(trie):
    node = trie.find("tri")
    suffixes = node.suffixes()
    print('is correct for tri? ', suffixes == ['e', 'gger', 'gonometry', 'pod'], suffixes)
    node = trie.find('ant')
    suffixes = node.suffixes()
    print('is correct for ant? ', suffixes == ['hology', 'agonist', 'onym', ''], suffixes)
